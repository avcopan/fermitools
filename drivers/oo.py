""" main drivers for the OO methods
"""
import numpy
import fermitools


def hf_orbitals(labels, coords, charge, spin, basis, interface):
...
def integrals(basis, labels, coords, interface):
...
def energy(method, labels, coords, charge, spin, basis, angstrom=False,
    ...
    h_ao, r_ao, p_ao = integrals(basis, labels, coords, interface)
    co_guess, cv_guess, no, nv = hf_orbitals(
            labels, coords, charge, spin, basis, interface)
