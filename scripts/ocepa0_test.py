...
def energy_driver(basis, labels, coords, charge, spin, niter=50,
    ...
    en_f = electronic_energy_functional(norb=2*nbf, nocc=na+nb, h_aso=h_aso,
                                        g_aso=g_aso, c=c)
    en_orb = functools.partial(en_f, t2=t2)

    t1 = numpy.zeros((na+nb, 2*nbf - (na+nb)))

    en_elec_test = en_orb(t1)
    print("test energy functional:")
    print(en_elec_test)
    print(en_elec)

    en_df = fermitools.math.central_difference(en_orb, t1, step=0.001, npts=13)
