...
COORDS = ((0.000000000000,  0.000000000000, -0.143225816552),
          ...
          (0.000000000000, -1.638036840407,  1.136548822547))
EN_DF = numpy.load(os.path.join(data_path, 'ocepa0/en_df.npy'))
...
def _main():
    ...
    # Numerically check the orbital gradient
    no = nocc
    nv = norb - nocc

    x = numpy.zeros(no * nv)
    t = numpy.ravel(fermitools.math.asym.ravel(t2, {0: (0, 1), 1: (2, 3)}))
    en_dx_func = solvers.oo.ocepa0.e_d1_f(
            norb=norb, nocc=nocc, h_aso=h_aso, g_aso=g_aso, c=c, npts=11)
    ...
    # en_f = solvers.oo.ocepa0.field_energy_solver(
