...
def fock(o, h, g):
    return h + numpy.einsum('piqi->pq', g[:, o, :, o])
...
def singles_residual(o, v, h, g, m1, m2):
    capf = (numpy.einsum('px,qx->pq', h, m1)
            + 1. / 2 * numpy.einsum('pxyz,qxyz->pq', g, m2))
    res1 = (capf - numpy.transpose(capf))[o, v]
    return res1
...
def main():
    ...
    en_nuc = fermitools.chem.nuc.energy(labels=LABELS, coords=COORDS)
    en = energy_routine(BASIS, LABELS, COORDS, CHARGE, SPIN)

    print('{:20.15f}'.format(en + en_nuc))
